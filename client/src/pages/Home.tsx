...
export default function Home() {
  ...
  return <main className="reference-home mx-auto max-w-[1080px] px-4 pb-12 pt-4 sm:px-8"><section className="reference-hero"><div className="reference-mark"><span className="mark-orbit">◉</span><strong>نغمة</strong><small>NAGHMAHUB</small></div><form onSubmit={submit} className="reference-search"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="إبحث عن اغنية او البوم او فنان" aria-label="البحث عن أغنية أو ألبوم أو فنان" /><button type="submit">بحث</button></form><div className="reference-trust"><ShieldCheck size={14} /> نتائج تجريبية آمنة · لا يتم تنزيل محتوى خارجي</div></section><section className="reference-list"><div className="reference-list-heading">نغمة</div>{links.map((item, index) => <Link key={`${item.slug}-${index}`} href={`/s/${item.slug}`} className="reference-list-item"><span>♫</span>{item.label}</Link>)}</section><section className="reference-footer-card"><p>مساحة عربية هادئة لاكتشاف الصوت.</p><div><Link href="/trending">جديد البحث</Link><Link href="/contact">اتصل بنا</Link><Link href="/dmca">DMCA</Link></div></section></main>;
}
