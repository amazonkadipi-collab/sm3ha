...
export default function Home() {
  ...
  return <main dir="rtl" className="reference-home mx-auto max-w-[1080px] px-4 pb-10 pt-3 sm:px-8">
    ...
    <section className="reference-list" aria-label="الأكثر بحثاً"><div className="reference-list-heading">نغمة</div>{isLoading && <div className="reference-loading">جارٍ تحميل الكلمات الأكثر بحثاً…</div>}{!isLoading && links.map((item, index) => <Link key={`${item.slug}-${index}`} href={workflowLinks.song(item.slug)} className="reference-list-item"><span>♫</span>{item.label}</Link>)}</section>
