...
export default function ArtistPage() {
  ...
  useEffect(() => { if (artist) { document.title = `${artist.name} | نغمة`; let jsonLd = document.getElementById("artist-jsonld"); if (!jsonLd) { jsonLd = document.createElement("script"); jsonLd.id = "artist-jsonld"; jsonLd.setAttribute("type", "application/ld+json"); document.head.appendChild(jsonLd); } jsonLd.textContent = JSON.stringify({ "@context": "https://schema.org", "@type": "MusicGroup", name: artist.name, url: window.location.href }); } return () => { document.title = "نغمة | NaghmaHub"; document.getElementById("artist-jsonld")?.remove(); }; }, [artist]);
