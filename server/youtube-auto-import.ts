...
function asPositiveInt(value: string | undefined, fallback: number, max: number) { const parsed = Number(value); return Number.isFinite(parsed) ? Math.min(Math.max(Math.floor(parsed), 1), max) : fallback; }
function splitQueries(value: string | undefined): string[] { return (value ?? "").split(/[\n,،]+/).map(item => item.trim()).filter(item => item.length >= 2 && item.length <= 120); }
...
async function loadSettings(): Promise<AutoImportSettings> {
  ...
  if (!supabase) return { enabled: asBoolean(process.env.YOUTUBE_AUTO_ENABLED, Boolean(process.env.YOUTUBE_API_KEY)), queries: envQueries.length ? envQueries : DEFAULT_QUERIES, maxQueries: asPositiveInt(process.env.YOUTUBE_AUTO_MAX_QUERIES, 5, 10), videosPerQuery: asPositiveInt(process.env.YOUTUBE_AUTO_VIDEOS_PER_QUERY, 10, 25), titleQueriesPerRun: asPositiveInt(process.env.YOUTUBE_AUTO_TITLE_QUERIES, 10, 20) };
...
async function recentSearchQueries(limit: number): Promise<string[]> {
  ...
  for (const row of data ?? []) { const query = String(row.query ?? "").trim(); if (query.length < 2 || query.length > 120) continue; const current = scores.get(query) ?? { count: 0, resultCount: 0 }; current.count += 1; current.resultCount += Number(row.result_count ?? 0); scores.set(query, current); }
  return Array.from(scores.entries()).sort((a, b) => b[1].count - a[1].count || b[1].resultCount - a[1].resultCount).slice(0, limit).map(([query]) => query);
}

async function filterNewRows(rows: YouTubeCatalogItem[]): Promise<YouTubeCatalogItem[]> {
  const supabase = getSupabaseAdmin();
  if (!supabase || !rows.length) return rows;
  const ids = Array.from(new Set(rows.map(row => row.providerVideoId)));
  const existing = new Set<string>();
  for (let offset = 0; offset < ids.length; offset += 100) { const chunk = ids.slice(offset, offset + 100); const { data } = await supabase.from("songs").select("provider_video_id").eq("provider", "youtube").in("provider_video_id", chunk); for (const row of data ?? []) existing.add(String(row.provider_video_id)); }
  return rows.filter(row => !existing.has(row.providerVideoId));
}

export async function runYouTubeAutoImport(): Promise<AutoImportResult> {
  const settings = await loadSettings();
  if (!settings.enabled) return { status: "disabled", queries: 0, scanned: 0, newRows: 0, accepted: 0, duplicates: 0 };
  if (!process.env.YOUTUBE_API_KEY) return { status: "not_configured", queries: 0, scanned: 0, newRows: 0, accepted: 0, duplicates: 0 };
  ...
  for (const query of queries) { try { const rows = await searchYouTubeVideos(query, settings.videosPerQuery); scanned += rows.length; newRows.push(...rows); } catch (error) { failures.push(`${query}: ${error instanceof Error ? error.message : "YouTube request failed"}`); } }
