...
type AnalyticsEventName = "page_view" | "search" | "song_view" | "media_view" | "conversion_start" | "admin_action";
...
export async function recordAnalyticsEvent(input: AnalyticsEventInput): Promise<boolean> {
...
export async function recordSearchLog(input: {
  query: string;
  path?: string;
  resultCount: number;
  songId?: string;
...
export async function getAnalyticsSummary(days: number) {
  const supabase = getSupabaseAdmin();
  if (!supabase) {
    return { available: false, days, totalEvents: 0, uniqueSessions: 0, eventBreakdown: [], topPaths: [], topSearches: [] };
  }
  ...
  return {
    ...
    uniqueSessions: new Set(eventRows.map(row => row.session_hash).filter(Boolean)).size,
    eventBreakdown: countBy(eventRows, "event_name"),
    topPaths: countBy(eventRows, "path"),
    topSearches: countBy(searchRows, "query"),
    recentSearches: searchRows.slice(0, 20),
  };
}
